...
def create_tables():
    ...
    cursor.execute("""
        CREATE TABLE new_knowledge (
            ...
            FOREIGN KEY (created_by) REFERENCES support_agent(id)
        )
    """)
    cursor.execute(
        "CREATE INDEX idx_new_knowledge_id ON new_knowledge(knowledge_id)")
    print("✓ Created new_knowledge table")
    ...
    # Show table info
    for table_name in ['realpage_user', 'support_agent', 'ticket', 'new_knowledge']:
