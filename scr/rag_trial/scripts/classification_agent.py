...
class ClassificationAgent:
    ...
    def __init__(self, vector_store_path="vector_store"):
        ...
        self.vector_store_path = Path(__file__).parent.parent / vector_store_path
    ...
    def _load_vector_store(self):
        ...
        if not Path(self.vector_store_path).is_absolute():
            base_path = Path(__file__).parent.parent  # Go to rag_trial directory
        ...
        print(f"✓ Using model: {self.metadata.get('model', 'text-embedding-3-small')}")
    ...
    def _generate_llm_response(self, query, retrieved_docs):
        ...
        context_parts = []
        for i, doc in enumerate(retrieved_docs[:3], 1):  # Use top 3 for context
            data = doc['data']
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python classification_agent.py 'your query here' [top_k] [--all]")
        print("\nExamples:")
    ...
    query = sys.argv[1]
    top_k = int(sys.argv[2]) if len(sys.argv) > 2 and sys.argv[2] != '--all' else 1
