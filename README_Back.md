...
|--------|------|-------------|-------------|
| `id` | INTEGER | PRIMARY KEY, AUTOINCREMENT | Unique identifier |
| `ticket_number` | VARCHAR(50) | UNIQUE, NOT NULL | Display ticket ID (e.g., "CASE-2024-001") |
| `conversation_id` | VARCHAR(100) | NOT NULL | From Excel - original conversation ID |
| `channel` | VARCHAR(50) | | From Excel - support channel |
...
| `first_tier_agent` | VARCHAR(100) | | From Excel - original agent name |
...
| `original_resolution` | TEXT | | RAG-generated resolution (immutable) |
| `edited_resolution` | TEXT | | Agent-edited resolution (if modified) |
| `relevancy_score` | FLOAT | | RAG confidence score (0.0 - 1.0) |
| `reference_articles` | TEXT | | JSON array: ["KB-001", "SCRIPT-005"] |
...
{
  "id": 1,
  "ticket_number": "CASE-2024-0001",
  "conversation_id": "CONV-12345",
  "channel": "Chat",
  "customer_role": "Property Manager",
  "product": "RealPage Accounting",
  "transcript": "Customer: I can't access my reports...",
  "first_tier_agent": "John",
  "status": "approved",
  "created_by": 1,
  "assigned_to": 1,
  "original_resolution": "Based on KB analysis...",
  "edited_resolution": "Updated solution: ...",
  "relevancy_score": 0.85,
  "reference_articles": "[\"KB-001\", \"KB-023\"]",
  "created_at": "2024-02-08T10:00:00Z",
  "updated_at": "2024-02-08T14:30:00Z",
  "resolved_at": "2024-02-08T14:30:00Z"
}
```
...
When processing a transcript, the RAG system returns:

```json
{
  "ticket_number": "CASE-2024-0001",
  "conversation_id": "CONV-12345",
  "created_date": "2024-02-08T10:00:00Z",
  "first_tier_agent": "John",
  "product": "RealPage Accounting",
  "resolution": {
    "content": "Based on our knowledge base analysis...\n\n**Root Cause:**\nThe user's session token has expired...\n\n**Recommended Solution:**\n1. Clear browser cache...\n2. Re-authenticate...",
    "relevancy_score": 0.85,
    "reference_articles": [
      {"id": "KB-001", "title": "Session Management Guide", "type": "kb"},
      {"id": "SCRIPT-005", "title": "Authentication Troubleshooting", "type": "script"}
    ]
  }
}
```
